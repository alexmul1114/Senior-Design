...
def main():
    ...
    if args.dataset.lower() == 'hrsid':
        ...
        train_patches_folder = os.path.join(args.dataset_path, 'yolo_4', 'train', 'images')
        val_patches_folder = os.path.join(args.dataset_path, 'yolo_4', 'val', 'images')
        test_patches_folder = os.path.join(args.dataset_path, 'yolo_4', 'test', 'images')
        train_labels_folder = os.path.join(args.dataset_path, 'yolo_4', 'train', 'labels')
        val_labels_folder = os.path.join(args.dataset_path, 'yolo_4', 'val', 'labels')
        test_labels_folder = os.path.join(args.dataset_path, 'yolo_4', 'test', 'labels')
        ...
        for annot in image_annotations:
            ...
            if box_x_start_idx == box_x_end_idx and box_y_start_idx == box_y_end_idx:  # No overlapping
                ...
                center_x, center_y, width, height = format_yolo(
                    ...
                    200, 
                    box_y_start % 200, 
                    mod_coord(box_y_end, 200)  # use mod_coord so exact multiples use 200, not 0
                )
                center_xs.append(center_x)
                center_ys.append(center_y)
                widths.append(width)
        ...
        for split in ['train', 'val', 'test']:
            image_split_folder = os.path.join(args.dataset_path, 'yolo_4', split, 'images')
            label_split_folder = os.path.join(args.dataset_path, 'yolo_4', split, 'labels')
        ...
        yaml_info = {"path": os.path.join(args.dataset_path, "yolo_4"), "train": "train/images", 
                     "val": "val/images", "test": "test/images", "names": {0: "ship"}}
        with open(os.path.join(args.dataset_path, 'yolo_4', args.dataset.lower() + '.yaml'), 'w') as yaml_file:
